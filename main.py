...
# Затем пользователь вводит сами элементы множеств.n = int(input("Введите размер N: "))
...
for i in range(m):
    List_2.append(int(input(f"Введите {i+1} элемент: ")))
...
# круглой грядке, причём кусты высажены только по окружности.
...
for i in range(n):
    bush.append(int(input(f"Введите количество ягод {i+1}: ")))

max = 0

for i in range(n):

    sum = bush[i-1]+bush[i]+bush[(i+1) % n]
    # print(i-1, i, (i+1)%n, sum)
    if sum > max:
        max = sum

print(bush)
print(f"Максимальное количество равно {max}")
